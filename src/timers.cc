...
bool loadMedia(LTexture* texture, SDL_Renderer* renderer, TTF_Font** font_ptr) {
	*font_ptr = TTF_OpenFont("fonts/lazy.ttf", 28);
	...
	if (!texture[0].loadFromRenderedText("Press Enter to Reset the Start Time.",
																		 textColor, renderer, *font_ptr))	{
		return false;
	}
	return true;
}
...
int main(int argc, char** argv) {
	...
	std::stringstream timeText; // Memory text stream

	/*
		Timers work by getting current time with SDL_GetTicks()
		This function returns the number of milliseconds that have passed since
	...
	while (!quit) {
		while (SDL_PollEvent(&e) != 0) {
			if (e.type == SDL_QUIT) {
				...
				startTime = SDL_GetTicks(); // Reset start time by making it curr time
			}
		}

		timeText.str(""); // Set time text as empty string
		// Fill stream with next text
		timeText << "Milliseconds since start time: " << SDL_GetTicks() - startTime;

		textures[1].loadFromRenderedText(timeText.str().c_str(), text_color,
																			 renderer, font);

		SDL_SetRenderDrawColor(renderer, 0xff, 0xff, 0xff, 0xff);
		SDL_RenderClear(renderer);

		// Render text centered with the prompt texture
		textures[0].render(renderer, (SCREEN_WIDTH - textures[0].getWidth()) / 2, 0);
		textures[1].render(renderer, (SCREEN_WIDTH - textures[0].getWidth()) / 2, 
									 (SCREEN_HEIGHT - textures[0].getHeight()) / 2);
